...
The design of the circuit also inlcuded a piezo buzzer and a switch. They were connected to the RedBoard via pins 5 and 2 respectively. When pin 5 sent out a signal, the buzzer would play a tone. Pin 2, whcih was connectted to the switch was set to input pullup mode. This meant that the the pin would read zero until the switch was moved, then it would read low. The input pullup resistor mode was used to prevent floating values.  
